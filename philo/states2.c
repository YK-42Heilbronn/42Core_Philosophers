...
/*   Updated: 2026/07/05 14:02:40 by ykonka           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	*first = &(philo_node->philo->fork_mutex);
	*second = &(philo_node->next_philo->philo->fork_mutex);
	if (*first > *second)
	{
		*first = &(philo_node->next_philo->philo->fork_mutex);
		*second = &(philo_node->philo->fork_mutex);
	}
